...
- 底部 / 顶部横向模式会把当前歌词和后续歌词拼成一条连续长轨道，每句之间使用 `4` 个中文字符宽度的间隔，并整体从右往左滑动
- 横向连续歌词会读取 LRC 时间戳，用“当前句到下一句”的时长控制滚动速度：当前句从进入中心卡片可视区到完全离开可视区的时间尽量与音频中的该句歌词时长一致；纯文本歌词没有时间戳时使用平滑兜底速度
- 左右吸附竖向模式同样使用连续歌词轨道：当前句和后续句会转成竖排，从下往上完整滑完；轨道运行期间不会因为当前句切换而清空重启，每句之间保留 `4` 个字符位的竖向间隔
